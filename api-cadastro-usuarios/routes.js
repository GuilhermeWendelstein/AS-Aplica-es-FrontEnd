...
function classificar(dados) {
  const { dor, febre } = dados;

  if (dor >= 8) {
    return { nivel: 'Alto', mensagem: 'Atendimento urgente' };
  }

  if (febre) {
    return { nivel: 'Médio', mensagem: 'Atenção recomendada' };
  }

  return { nivel: 'Baixo', mensagem: 'Caso leve' };
}
